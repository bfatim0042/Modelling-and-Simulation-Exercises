//#-- ========================================================================================
//#--                Exercise 8 - Bea Fatima, Student ID: 1725181
//#-- ========================================================================================
#include <stdio.h>
#include <time.h>
#include <assert.h>
...
measurement_t measure(void){ 
    ...
    /*--------- Your code goes here -----------*/
    //must define average_pressure and mu_excess

   double volume = box[0] * box[1] * box[2]; // volume of simulation box
   double rho = n_particles / volume; //density
   double kbT = 1.0 / beta; //beta = 1/k_b*T

   //the virial term is already calculated, substitute into equation
    result.average_pressure = rho * kbT + (1/(3.0 * volume)) * virial;

    //start widom method for excess chemical potential
      double sum = 0.0;
    ...
    for(int i = 0; i < Ntest; i++){
        ...
        //create random positon and scale by box size
        for(int d = 0; d < NDIM; d++){
            randpos[d] = box[d] * dsfmt_genrand();
        }
    ...
    // average sum over test particles
    double average = sum / Ntest;

    // calculate excess chemical potential
    if(average > 0.0)
        result.mu_excess = -kbT * log(average);
    else
        result.mu_excess = 0.0;
